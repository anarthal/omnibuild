...
class CppTargetProperties(object):
    def __init__(self, compiler):
        self.compiler = compiler
        self.includes = []

class ObjectFile(Target):
    def __init__(self, src, dirstack, properties):
        ...
        self.includes = self.properties.compiler.get_includes(self.full_src)
    def is_up_to_date(self, cache):
    ...
    def build(self):
        self.properties.compiler.compile(self.full_src, self.output, self.properties.includes)
    def update_cache(self, cache):
...
class CppTarget(Target):
    def __init__(self, name, sources, dirstack, compiler):
        ...
        self.properties = CppTargetProperties(compiler)
        self.objects = [ObjectFile(src, dirstack, self.properties) for src in sources]
        self.libs = []
    ...
    def link_library(self, lib):
        self.link_libraries([lib])
    def link_libraries(self, libs):
        to_add = libs
        for lib in libs:
            to_add += lib.libs
            self.properties.includes += lib.properties.includes
        self.libs += to_add
        self.depends += to_add
        
    def add_include_directories(self, dirs):
        self.properties.includes += [os.path.abspath(self.dirstack.join_real(d)) for d in dirs]
    def add_include_directory(self, directory):
        self.add_include_directories([directory])
...
class Executable(CppTarget):
    def build(self):
        self.properties.compiler.link_executable(self._get_link_inputs(), self.output_file)

class DynamicLibrary(CppTarget):
    def build(self):
        self.properties.compiler.link_dynamic_library(self._get_link_inputs(), self.output_file)
    def _get_output_file(self, name, dirstack):
